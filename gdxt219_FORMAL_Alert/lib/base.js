    ...
    */
    ...
    openContractPage: function(data,elem,document){
      if (data.eventType === 'toReturnClick' || data.eventType === 'imgContractClick' || data.eventType === "btnClick") {
        ...
        var url = "http://10.1.24.207:7001/grxd/jsp/yq.jsp?applSeq=" + idNo + apptSeq + "&idNo=" + idNo;
        if(data.eventType === "imgContractClick"){
          url = "http://10.1.24.207:7001/grxd/jsp/yq.jsp?applSeq=" + applSeq + "&idNo=" + idNo;
        }
        ...
        xhr.onreadystatechange = function () {
          if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            var url_real = 'http://10.1.235.135:28080/mmecserver3.0/showContract.do?';
        ...
        if(/bench/.test(aWin.location.href)){
          ...
          if(_win.checkMenu){
            if(_win.document.querySelector("#cld_loan")){
              _win.checkMenu('cld_loan','http://10.1.24.207:7001/grxd/jsp/frame/left28.jsp','/grxd/jsp/success.htm');
            }else{
              // 不存在“云按揭贷款”菜单
        ...
        if(/(pageName=login)|(login\.jsp)/.test(_url)){
          ...
          if(top.switch===undefined){
            try{
              ...
              setTimeout(function(){
                top.switch="done";
                ysp.appMain.getActiveWindow().location.href = "http://10.1.24.207:7001/grxd/SsoLServlet";
								// yspUser.getPassword("http://10.1.24.207:7001/grxd/DirectForwardAction.do?pageName=login")
								// yspUser.getUsername("http://10.1.24.207:7001/grxd/DirectForwardAction.do?pageName=login")
              },100)
              // alert()
            }catch(e){
          	}
          ...
          var loginTarget=setInterval(function(){
            if(doc.querySelector("#loginForm") && doc.querySelector("#login_uid") && doc.querySelector("#login_upwd") && doc.querySelector("#submitBtn")){
              ...
              if(top.switch==="done"){
                try{
                  var username=yspUser.getUsername("http://10.1.24.207:7001/grxd/DirectForwardAction.do?pageName=login");
                  var password=yspUser.getPassword("http://10.1.24.207:7001/grxd/DirectForwardAction.do?pageName=login");
