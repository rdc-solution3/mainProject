...
/*updated by graysky @ 2008-03-20*/
// Node object
function Node(id, pid, name, url, isshow, title, target, icon, iconOpen, open) {
	this.id = id;
	this.nodeId=0;
	this.pid = pid;
	this.name = name;
...
function dTree(objName) {
	...
	this.showradioboxflag = 0;          // if radiobox is show  
	...
	this.icon = {
		root		: _path+'/myTree/img/base.gif',
		folder		: _path+'/myTree/img/folder.gif',
		folderOpen	: _path+'/myTree/img/folderopen.gif',
		node		: _path+'/myTree/img/page.gif',
		empty		: _path+'/myTree/img/empty.gif',
		line		: _path+'/myTree/img/line.gif',
		join		: _path+'/myTree/img/join.gif',
		joinBottom	: _path+'/myTree/img/joinbottom.gif',
		plus		: _path+'/myTree/img/plus.gif',
		plusBottom	: _path+'/myTree/img/plusbottom.gif',
		minus		: _path+'/myTree/img/minus.gif',
		minusBottom	: _path+'/myTree/img/minusbottom.gif',
		nlPlus		: _path+'/myTree/img/nolines_plus.gif',
		nlMinus		: _path+'/myTree/img/nolines_minus.gif'
	};
...
dTree.prototype.toString = function() {
	if (this.config.stepDepth < 0 || (this.showchkboxflag==1 && this.checksubflag==1) || (this.showradioboxflag==1 && this.checksubflag==1)) {
...
dTree.prototype.node = function(node, nodeId) {
	...
	if (this.config.useIcons) {
        ...
        var ss = (this.showchkboxflag == 1) ? 'style="display:"' : '';
		str += '<img '+ ss +' id="i' + this.obj + nodeId + '" src="' + ((node._io) ? node.iconOpen : node.icon) + '" alt="" />';
        
        //CheckBox
        if(node.pid!=-2 && this.showchkboxflag==1 && node.isshow=="Y") {
			str += '<input style="margin:0;padding:0;width:18px;height:18px;vertical-align:middle" type="checkbox" name="ckx' + nodeId + '"';
			str += ' onclick="javascript: ' + this.obj + '.ck(' + nodeId + ');" >';
			
			node.nodeId = nodeId;
		}
		//RadioBox
		 if(node.pid!=-1 && this.showradioboxflag==1 && node.isshow=="Y") {
		 	
			str += '<input style="margin:0;padding:0;width:18px;height:18px;vertical-align:middle" type="radio" id="rad_'+node.id+'" name="rad"  desc="'+node.name+'"';
			str += ' dd="'+node.other+'">';// onclick="javascript:'+this.obj+'.rk("'+node.id+'");"
			node.nodeId = nodeId;
		}
	...
	if (node.url) {
		...
		//onclick="javascript:'+this.obj+'.rk('+nodeId+')"
	} else{
		str += '<a href="javascript:void(0);">';
	}
...
//have nextnode by id
dTree.prototype.readKit = function(id){
	for(var n=0; n<this.aNodes.length; n++) {
	    var node = this.aNodes[n];
	    if(node.pid == id) {
		    return true;
...
//radio clicked to do 
dTree.prototype.rk = function(id) {
	var obj1=document.getElementById("zoneID");
	var obj2=document.getElementById("zoneName");
	
	if (!this.readKit(id)){
		obj1.value=id;
		obj2.value=this.getNodeById(id).name;
	}else{
		obj1.value=0;
		obj2.value="未分配"
	}
...
// Open by input name
dTree.prototype.oByNodeName = function(tname){
	var temp=false;
	for (var n=0; n<this.aNodes.length; n++) {
		var node = this.aNodes[n];
		if (node.name==tname) {
			this.openTo(node.id,true);
			temp=true;
		}
	}
